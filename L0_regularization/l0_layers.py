...
class L0Activation(Module):
    ...
    def sample_z(self, sample=True):
        ...
        if sample:
            eps = self.get_eps(self.floatTensor(self.in_features))
...
class L0Conv2d(Module):
    ...
    def forward(self, input_, input_weights=None, input_qz_loga=None):
        ...
        self.weights = nn.Parameter(input_weights) if input_weights is not None
        self.qz_loga = nn.Parameter(input_qz_loga) if input_qz_loga is not None

        if self.input_shape is None:
            self.input_shape = input_.size()
        b = None if not self.use_bias else self.bias
        if self.local_rep or not self.training:
            output = c(input_, self.weights, b, self.stride, self.padding, self.dilation, self.groups)
            z = self.sample_z(output.size(0), sample=self.training)
            return output.mul(z)
        else:
            weights = self.sample_weights()
            output = c(input_, weights, None, self.stride, self.padding, self.dilation, self.groups)
            return output
